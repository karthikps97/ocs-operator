...
func TestGetKubeResourcesForClass(t *testing.T) {
	...
	for idx, obj := range objs {
		objIdxByName[obj.object.GetName()] = idx
	}
	...
	for _, expName := range []string{"class-1", "class-2"} {
		t.Run(expName, func(t *testing.T) {
			...
			wantObj.Name = expName
			if !equality.Semantic.DeepEqual(gotObj.object, wantObj) {
				t.Fatalf("expected %v to be deep equal to %v", gotObj.object, wantObj)
			}
		})
	}
}
